...
__always_inline uint64_t get_clocktick_ns(clockid_t clock) {
    struct timespec ts;
...
mi_smallint yield_ms(mi_smallint ms) {
    uint64_t  now,tgt;
    struct timespec sleep_tm,rem_tm;
    
    sleep_tm.tv_sec=0;
    sleep_tm.tv_nsec=10000000;


    now = get_clocktick_ns(CLOCK_REALTIME);
    tgt = now + (ms*1000000);
    while( get_clocktick_ns(CLOCK_REALTIME) < tgt) {
        mi_yield();
        if( nanosleep(&sleep_tm,&rem_tm) < 0 ) {
            return 0;
        }
    }
    return 0;
...
void yield_ms_proc(mi_smallint ms) {
    uint64_t  now,tgt;
    struct timespec sleep_tm,rem_tm;
